...
def MG_ini_data_generating():
    ...
    for idx1 in THETA1_INITIAL_RANGE:
        for idx2 in THETA2_INITIAL_RANGE:
            ini_state.append([idx1, idx2, 0, 0]) # 2*4
    ...
    for idx in range(NUM_INI_STATE):
         for i in range(MULTI_GUESS):
               ...
               random_theta_guess_list[idx, i, :]= [theta1_guess, theta2_guess, 0, 0]
    random_ini_theta_guesses = random_theta_guess_list # 50*4
    ...
    return random_ini_theta_guesses, ini_state, data_idx
...
def original_MG_initial_state_loop(random_ini_theta_guesses:float, x0_state:np.array, idx_group_of_control_step:int,
    ...
    # temp data
    temp_x_next = np.zeros((MULTI_GUESS, nx))
    temp_u_solve = np.zeros((MULTI_GUESS, nu))
    temp_X_horizon = np.zeros((MULTI_GUESS, N, nx))
    temp_U_horizon = np.zeros((MULTI_GUESS, N-1, nu))
    temp_cost = np.zeros((MULTI_GUESS, 1))


    for i in range(0, CONTROL_STEPS):    
         for idx in range(MULTI_GUESS):
             # initial guess
             if i == 0:
                  x_guess = random_ini_theta_guesses[idx,:]
                  u_guess = 0
                  ...
                  ocp_solver.set(N-1, "x", x_guess[N-1,:])
                  ocp_solver.set(N, "x", x_guess[N-1,:])
             ...
             # temp x next 
             temp_x_next[idx,:] = ocp_solver.get(1, "x")

             # temp u solve
             temp_u_solve[idx, :] = ocp_solver.get(0, "u")  

             # temp x, u horizon 
             for k in range(N-1):
                  x_guess_k = ocp_solver.get(k + 1, "x")
                  u_guess_k = ocp_solver.get(k + 1, "u")
                  temp_X_horizon[idx,k,:] = x_guess_k 
                  temp_U_horizon[idx,k,:] = u_guess_k 
             x_guess_last = ocp_solver.get(N, "x")
             temp_X_horizon[idx,N-1,:] = x_guess_last

             # temp cost
             temp_cost[idx,0] = ocp_solver.get_cost()

         # find the min cost 
         min_value = np.min(temp_cost)
         min_index = np.argmin(temp_cost)

         # solve control signal u
         u_solve = temp_u_solve[min_index,:]
         U_result[i,:] = u_solve
         ...
         # derive next state
         x_next = temp_x_next[min_index,:]
    
         # previous solution as initial guess for warm start
         for k in range(N-1):
             x_guess_k = temp_X_horizon[min_index,k,:]
             u_guess_k = temp_U_horizon[min_index,k,:]
         ...
         x_guess_last = temp_X_horizon[min_index,N-1,:]
         X_horizon[i,N-1,:] = x_guess_last
         # ocp_solver.set(N, "x", x_guess_last)
         # X_horizon[i,N-1,:] = x_guess_last

         # next MG
         x_MG, u_MG = next_MG_guesses_generating(X_horizon[i,:,:], U_horizon[i,:,:])
...
########## original single state control loop ########## 
def noise_state_single_loop_MG(noisy_data, noisy_data_x_guess, noisy_data_u_guess, idx_noise_data, ctl_step, idx_group_of_control_step):
    ########## create Acados solver ##########
    ...
    for idx in range(MULTI_GUESS):
        if ctl_step == 0:
            x_guess = noisy_data_x_guess[idx,:]
            u_guess = 0
            ocp_solver.set(0, "u", u_guess) 
            ocp_solver.set(0, "x", x_guess)
        else:
            x_guess = noisy_data_x_guess[idx,:,:]
            u_guess = noisy_data_u_guess[idx,:,:]
            for k in range(N-1):
                x_guess_k = x_guess[k,:]
                u_guess_k = u_guess[k,:]
        ...
        if status != 0:
            print(f"Solver failed at step with status {status}")
    ...
    # solve control signal u
    noise_u_solve = temp_u_solve[min_index,:]
...
def RunMPCForSingle_IniState_IniGuess(random_ini_theta_guesses:float, x0_state:np.array, idx_group_of_control_step:int,
    ...
    try:
        ...
        x_ini_memory, u_ini_memory, j_ini_memory, X_horizon, U_horizon = original_MG_initial_state_loop(random_ini_theta_guesses, x0_state, idx_group_of_control_step, u_ini_memory, x_ini_memory, j_ini_memory)
        ...
        for ctl_step in range(CONTROL_STEPS):
                ...
                if ctl_step == 0:
                    current_x_guess = random_ini_theta_guesses
                    current_u_guess = 0
                    noisy_data = states_noise_generating(current_states)
                    ...
                    noisy_data = states_noise_generating(current_states)
                    noisy_data_x_guess, noisy_data_u_guess = next_MG_guesses_generating(current_x_guess, current_u_guess)
                ...
                for n in range(NUM_NOISY_DATA):
                    ...
                    noisy_x_guesses = noisy_data_x_guess
                    noisy_u_guesses = noisy_data_u_guess

                    # print(f'noisy_state -- {noisy_state_n}')
                    # print(f'noisy_u_guess -- {noisy_u_guess}')

                    U_step_result, cost_step_result = noise_state_single_loop_MG(noisy_state_n, noisy_x_guesses, noisy_u_guesses, n, ctl_step, idx_group_of_control_step)
...
def main():
    # ini data generating
    random_ini_theta_guesses, ini_state, data_idx = MG_ini_data_generating() 
    ...
    for n in range(NUM_INI_STATE):
          initial_data_groups.append([random_ini_theta_guesses[n,:,:], ini_state[n,:], data_idx [n], u_ini_memory, u_random_memory, x_ini_memory, x_random_memory, j_ini_memory, j_random_memory])
