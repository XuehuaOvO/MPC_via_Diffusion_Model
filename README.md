...
In this work, we propose a diffusion model that learns the data distribution generated by Model Predictive Control (MPC), enabling a near-globally optimal approximation of control policies for completing control tasks. Within the MuJoCo simulation environment, we train diffusion models to replace MPC in controlling the Franka Panda robot. The goal is to efficiently guide the gripper base to a specified target position in space.

The following video and figure illustrate the control process and the resulting trajectory of the gripper base:
