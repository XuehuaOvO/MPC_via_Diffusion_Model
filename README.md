...
In this work, we propose a diffusion model that learns the data distribution generated by Model Predictive Control (MPC), enabling a near-globally optimal approximation of control policies to finish the control task. In the MuJoCo simulation environment, we trained diffusion models to replace MPC to control the Franka Panda to efficiently move its gripper base to reach the target position in the space:
...
mv ~/MPC_via_Diffusion_Model/isaac-gym-preview-4 ~/MPC_via_Diffusion_Model/deps/isaac-gym-preview-4
...
# gdown --folder https://drive.google.com/drive/folders/1bx9IPdXFUOJKGZH2pNMQEV-KxtaXmDiH -O training_data/CartPole-NMPC # download Cart Pole training data
gdown --folder https://drive.google.com/drive/folders/1cnuGUnV4RENQuLEkK49t-mGV3OgQQKZw -O training_data/Panda-Data/panda_test6 # download Franka Panda training data

```

## Running inference with pre-trained diffusion model

```bash
conda activate mpd

cd MPC_via_Diffusion_Model/scripts/Panda/panda_inference/
python inference_diffusion_panda.py 
```
By running the py file, you can see the mujoco viewer simulation of moving the Franka Panda gripper base to the target position by using the pre-trained diffusion model.
